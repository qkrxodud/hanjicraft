...
| 2026-09-26 | 리디자인 전면 적용 (Claude Design `홍현정한지공예 리디자인.dc.html`) | styles/style.css(전면 재작성), styles/artwork-detail.css, app/page.tsx, app/layout.tsx, app/globals.css, components/{Nav,Hero,ClientWrapper,ArtworkDetailClient,PopupNaver}.tsx, lib/translations.ts | 강조색 골드#C9A96E→단청 홍#8C2B2B, 서체 Cormorant+Roboto→Gowun Batang(제목)+Noto Serif KR(본문)+Cormorant(라틴 라벨), 전면 유동 레이아웃(clamp+auto-fit, 브레이크포인트는 헤더만), 히어로를 상태 기반 크로스페이드(Hero.tsx)로 분리, 구매 FAB→푸터 SHOP 칼럼, 프리로더·커스텀커서·패럴랙스 제거. 다국어(KO/EN/FR)·접근성·상세 페이지 라우팅은 유지 |
| 2026-09-27 | 고풍 장치 추가 + 렌더링 성능 | components/{Hero,ClientWrapper,ArtworkDetailClient,SWRegister}.tsx, app/{page,layout}.tsx, styles/style.css, styles/artwork-detail.css, public/sw.js | 고풍: 히어로 관지(세로쓰기+낙관 紙), 광곽 이중 괘선(모바일 플레이트), 장 번호 한자(一~八), 인용 「」, 판심 마름모 장식, 푸터 낙관. 성능: body opacity:0+JS 게이트 → CSS pageFadeIn 애니메이션(FCP 6.2s→0.19s), 이탈 전환은 page-leaving 클래스로 대체, content-visibility:auto(화면 밖 섹션), 서비스 워커 cache-first(이미지+/_next/static, GH Pages max-age=600 보완, VERSION 올려 무효화) — 재방문 이미지 transferSize 0 확인 |
| 2026-09-27 | 모바일 감상 지면 재구성 + 한지 질감 | styles/style.css, app/layout.tsx, app/page.tsx, components/ClientWrapper.tsx | 전역 .paper-texture(닥섬유 결 3겹 multiply)·body 농담 그라디언트, ≤760px에서 작품을 배접 플레이트(밝은 종이 테두리+낮은 그림자)로 제시, 원본 비율 그대로 노출(자르기 제거, 마크업 w/h로 CLS 방지), 섹션 머리글 중앙+판심 괘선, 캡션 중앙 정렬+괘선, 작품 간격 44~64px, 이미지 등장을 스케일 다운 페이드로, 560~760px 2열에서는 1:1 유지. 빠른 스크롤 시 .reveal 미노출 버그 수정(top<0 요소 강제 노출) |
