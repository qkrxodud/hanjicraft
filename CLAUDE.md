...
**목표:** hanji-craft(홍현정한지공예 연구소) 사이트의 프론트엔드·i18n·Spring Boot 백엔드 작업을 전담 에이전트 팀이 조율하여 일관성 있게 개발한다.

**트리거:** hanji-craft 관련 작업(HTML 수정, 갤러리, i18n 번역, 컨트롤러, 배포 등)이 있으면 `hanji` 스킬을 사용하라. TDD 백엔드 작업에는 `hanji-tdd` 스킬도 활용. 단순 질문은 직접 응답 가능.
